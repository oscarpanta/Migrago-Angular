...
export class RegistroComponent implements OnInit,OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('nextseccion') == 'true') {
      const datauser = localStorage.getItem('loggedInUser')
      ...
      this.miFormulario.get('nombre')?.setValue(data.given_name);
      this.miFormulario.get('apellidos')?.setValue(data.family_name);
      this.miFormulario.get('email')?.setValue(data.email);
      ...
      this.seccion++
      localStorage.removeItem('nextseccion')
    }
  ...
  registro() {
    ...
    this.miFormulario.get('password')?.setValidators(passwordValidators);
    this.miFormulario.get('password')?.updateValueAndValidity();
...
*/
  ...
  navigateToFormStep(stepNumber: number): void {
    // Oculta todos los pasos del formulario
    document.querySelectorAll(".form-step").forEach((formStepElement: Element) => {
      formStepElement.classList.add("d-none");
    });
    // Muestra el paso específico
    document.querySelector("#step-" + stepNumber)?.classList.remove("d-none");
    // Marca el círculo del paso como activo
    const formStepCircle = document.querySelector(`li[step="${stepNumber}"]`);
    formStepCircle?.classList.remove("form-stepper-unfinished", "form-stepper-completed");
    formStepCircle?.classList.add("form-stepper-active");
  }
