...
export class ModalRegistroComponent implements OnInit {
  @Output() openModalLogin: EventEmitter<void> = new EventEmitter<void>();
  @Input() dataPasar!: any;
  ...
  ngOnInit(): void {
    setTimeout(() => {
     ...
     if(this.dataPasar){
      ...
      this.miFormularioRegistro.get('nombre')?.setValue(this.dataPasar.nombre);
      this.miFormularioRegistro.get('apellidos')?.setValue(this.dataPasar.apellido);
      this.miFormularioRegistro.get('email')?.setValue(this.dataPasar.email);
      this.miFormularioRegistro.get('password')?.clearValidators();
      this.miFormularioRegistro.get('password')?.updateValueAndValidity();
      this.tipolog=1
      this.seccion=this.dataPasar.nextseccion

     }
     else{
      console.log('no hay datapassar')
     }
  ...
  registroUsuario() {
    ...
    this.miFormularioRegistro.get('password')?.setValidators(passwordValidators);
    this.miFormularioRegistro.get('password')?.updateValueAndValidity();
    ...
    siguienteSection() {
      ...
      const { email } = this.miFormularioRegistro.value;
      const requestData = {
        email: email
      };

      this.authService.usuarioEmail(requestData).subscribe(
        response => {
          if(!response.error){
            this.seccion++;
          }
          ...
          .subscribe((response: any) => {
            ...
            if (response) {
              if (response.error) {
                ...
                // this.miFormulario.get('nombre')?.setValue(calificacion);
                this.miFormularioRegistro.get('nombre')?.setValue(payLoad.given_name);
                this.miFormularioRegistro.get('apellidos')?.setValue(payLoad.family_name);
                this.miFormularioRegistro.get('email')?.setValue(payLoad.email);
                this.miFormularioRegistro.get('password')?.clearValidators();
                this.miFormularioRegistro.get('password')?.updateValueAndValidity();
